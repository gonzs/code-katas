...
const testCases = {
  pricesBtc: [39, 18, 29, 25, 34, 5, 29],
  pricesUbi: [39, 18, 29, 25, 34, 32, 5],
...
function maxProfit(prices) {
  let profitsArray = [];
  const getProfit = (pricesArray) => {
    const purchasePrice = pricesArray[0];
    let profit = 0;
    let newPricesArray = [];
    for (let i = 1; i < pricesArray.length; i++) {
      newPricesArray.push(pricesArray[i]);
      const auxProfit = pricesArray[i] - purchasePrice;
      if (auxProfit > profit) profit = auxProfit;
    }
    profitsArray.push(profit);
    if (newPricesArray.length > 1) getProfit(newPricesArray);
  };

  getProfit(prices);
  const bestProfit = profitsArray.sort((a, b) => b - a)[0];
  return bestProfit > 0 ? bestProfit : -1;
}

Object.values(testCases).forEach((prices) => {
