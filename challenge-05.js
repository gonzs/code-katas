...
function daysToXmas(date) {
  ...
  const lastDay = (d) => {
    const auxDate = new Date(d.getFullYear(), d.getMonth() + 1, 0);
    return auxDate.getDate();
  };

  let daysCountDown = 0;
  if (xmas.getFullYear() > date.getFullYear()) {
    daysCountDown += (xmas.getFullYear() - date.getFullYear()) * 365;
    daysCountDown += (xmas.getMonth() - date.getMonth()) * 30;
  } else if (xmas.getFullYear() < date.getFullYear()) {
    daysCountDown += -(date.getMonth() * 30);
    daysCountDown += -(lastDay(xmas) - xmas.getDate() + date.getDate());
  } else {
    daysCountDown = xmas.getDate() - date.getDate();
  }

  return daysCountDown;
}

const expectedResults = [24, 1, 1, 5, 0, -1, -6, -7, -7, 395, -50];
const testDates = [
  new Date("Dec 1, 2021"),
  ...
  new Date("Feb 14, 2022"),
];

const results = testDates.map((date) => daysToXmas(date));
