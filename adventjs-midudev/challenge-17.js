...
Necesitamos saber el número de paquetes que una persona va a poder gestionar en un día. Para ello se cuenta el número de paquetes que puede llevar 
esa persona y todos los transportistas que tiene en su equipo. Lo malo es que los datos están almacenados de una forma un poco rara en un array:
...
function countPackages(carriers, carrierID) {
  let countedPackages = 0;

  const countPackagesById = (employeeId) => {
    const index = carriers.findIndex(([id]) => id === employeeId);
    const [_, count, hierarchy] = carriers[index];

    countedPackages += count;

    if (hierarchy.length !== 0) hierarchy.forEach(countPackagesById);
  };

  countPackagesById(carrierID);

  return countedPackages;
}
...
const count1 = countPackages(carriers1, "dapelu");
console.log(count1);
const count2 = countPackages(carriers2, "camila");
console.log(count2);
